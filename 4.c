...
struct Node* pushAtEnd(struct Node* head, int value) {
    struct Node* newNode = createNode(value);
    if (head == NULL) {
        return newNode;  // If the list is empty, new node is the head
    }
...
int main() {
    ...
    printf("Linked list before pushing 8 at the end: ");
    printList(head);

    head = pushAtEnd(head, 8);  // Push 8 at the end

    printf("Linked list after pushing 8 at the end: ");
    printList(head);
